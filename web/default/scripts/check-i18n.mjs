...
/*
 * i18n completeness validator (default theme).
 *
 ...
 *
 * Source keys for the default theme are the ENGLISH strings; the base locale
 * is en.json. The check is intentionally MISSING-KEY only — it does NOT flag
 * value===key "untranslated" entries (too many legitimate brand/cognate cases
 * such as OAuth, Passkey, Section, etc.).
 *
 * Run from web/default/:  node scripts/check-i18n.mjs   (or: bun run i18n:check)
 */
...
async function main() {
  ...
  // Summary header.
  console.log('i18n completeness check (default theme)')

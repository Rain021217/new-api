...
describe('default affiliate trend helpers', () => {
  test('builds daily trend rows with stable labels and bar widths', () => {
    ...
    assert.equal(rows[1].effectiveNewUsers, 2)
    assert.equal(rows[1].estimatedCommissionRmb, 4.56)
  })

  test('returns an empty list when summary has no trend points', () => {
    assert.deepEqual(buildAffiliateTrendRows(undefined), [])
    assert.deepEqual(buildAffiliateTrendRows({ daily_trends: [] }), [])
  })
})

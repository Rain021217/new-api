...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		affiliateAdminRoute.Use(middleware.AdminAuth())
		{
			...
			affiliateAdminRoute.GET("/rule-sets/default-seed", controller.AdminGetAffiliateRuleSetDefaultSeed)
			...
			affiliateAdminRoute.PATCH("/commissions/:id/void", controller.AdminVoidAffiliateCommissionEvent)
		...
		// WeChat scan login (QR code) — reuses the same external WeChat program as the code login
		apiRouter.POST("/oauth/wechat/login/qrcode", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.WeChatLoginQRCode)
		apiRouter.GET("/oauth/wechat/login/qrcode/image", middleware.CriticalRateLimit(), controller.WeChatLoginQRCodeImage)
		apiRouter.GET("/oauth/wechat/login/status", middleware.CriticalRateLimit(), controller.WeChatLoginStatus)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			userRoute.POST("/sms/register/code", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.SendSMSRegisterCode)
			userRoute.POST("/sms/login/code", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.SendSMSLoginCode)
			userRoute.POST("/sms/register", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.SMSRegister)
			userRoute.POST("/login", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.Login)
			userRoute.POST("/login/phone", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.SMSPhoneLogin)
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.POST("/sms/bind/code", middleware.CriticalRateLimit(), controller.SendSMSBindCode)
				selfRoute.POST("/sms/bind", middleware.CriticalRateLimit(), controller.SMSBindPhone)
				selfRoute.POST("/wechat/bind/qrcode", middleware.CriticalRateLimit(), controller.WeChatLoginBindQRCode)
				selfRoute.GET("/wechat/bind/status", middleware.CriticalRateLimit(), controller.WeChatLoginBindStatus)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.POST("/sms/test", controller.AdminTestSMS)
			optionRoute.GET("/sms/status", controller.AdminGetSMSStatus)
